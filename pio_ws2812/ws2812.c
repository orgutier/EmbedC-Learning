 ...
 */
 ...
 void pattern_allstripregresive(uint len, uint t) {
     uint shift = 4;
...
uint8_t logicled = 0;
void pattern_counter(uint len, uint t) {
    logicled++;
    uint8_t indexed = 0;
    currentcolor++;
    if (currentcolor > maxcolor){
    ...
    for (uint i = 0; i < len; ++i) {
        indexed = i & 0x07; // 0b 0000 0111 mask, from 0 to 7
        uint8_t mask = (0x01<<indexed) & logicled;
        const uint8_t low = 0x0f;
        const uint8_t high = 0xaa;
        if (mask){ // Check if indexed'th bit of i is 1
            if (currentcolor == 0)
                put_pixel(urgb_u32(low, low, high));
            if (currentcolor == 1)
                put_pixel(urgb_u32(low, high, low));
            if (currentcolor == 2)
                put_pixel(urgb_u32(high, low, low));
            if (currentcolor == 3)
                put_pixel(urgb_u32(high, high, high));
        }
        else{
            put_pixel(urgb_u32(low, low, low));
        }
    }
}
 ...
 const struct {
     ...
     const char *name;
         ...
         {pattern_greys,   "Greys"},
         {pattern_allstripregresive, "All strip regresive"},
 ...
 void output_strips_dma(value_bits_t *bits, uint value_length) {
  ...
  int main() {
    ...
    const uint GATE_control = 15;
    const uint voltagetestpin = 22;
    gpio_init(GATE_control);
    gpio_set_dir(GATE_control, GPIO_OUT);
    gpio_put(GATE_control, 0);

    //gpio_init(voltagetestpin);
    //gpio_set_dir(voltagetestpin, GPIO_OUT);
    //gpio_put(voltagetestpin, 0);
     ...
     while (1) {
         ...
         for (int i = 0; i < 1000; ++i) {
             ...
             pattern_table[5].pat(NUM_PIXELS, t);
             //current_strip_out = strip1.data;
             //current_strip_4color = true;
             //pattern_table[pat].pat(NUM_PIXELS, t);
 
             transform_strips(strips, count_of(strips), colors, NUM_PIXELS * 3, brightness);
             dither_values(colors, states[current], states[current ^ 1], NUM_PIXELS * 3);
             sem_acquire_blocking(&reset_delay_complete_sem);
             output_strips_dma(states[current], NUM_PIXELS * 3);
 
             //sleep_ms(2000);
             current = 0;
             t += dir;
             brightness = (0x20 << FRAC_BITS) -1;
