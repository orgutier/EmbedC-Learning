 ...
 */
 ...
 int main() {
     ...
     bool success = pio_claim_free_sm_and_add_program_for_gpio_range(&ws2812_program, &pio, &sm, &offset, CUSTOM_PIN, 1, true);
     hard_assert(success);
 
     ws2812_program_init(pio, sm, offset, CUSTOM_PIN, 800000, IS_RGBW);
